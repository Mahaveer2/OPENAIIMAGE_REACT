  ...
  const generateImage = async() => {
    if(form.prompt){
      try {
        ...
        const response = await fetch("https://fine-lime-termite-wear.cyclic.app/api/v1/dalle",{
          method:"POST",
          headers:{
            "Content-Type":"application/json"
          },
      ...
      <form className="mt-16 max-w-3xl" onSubmit={handleSubmit}>
        <div className="flex flex-col gap-5">
          ...
          <div className="relative bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 w-64 p-3 h-64 flex justify-center items-center">
            {form.photo ? (
              <img src={form.photo} alt={form.alt} className="w-full h-full object-contain"/>
            ) : (
        ...
        <div className="mt-5 flex gap-5">
          <button className="text-white bg-green-700 font-medium rounded-md text-sm sm:w-auto w-full px-5 py-2.5 text-center" type="button" onClick={generateImage}>
